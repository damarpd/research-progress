...
- **Key feature analysis**: 
    ...
    - Below indicates observed pedestrian walking speed by several categories.
...
- **Enhanced social force model**: 
    ...
    - Improvement made by incorporating **group-type categorization** -> improved pairing behavior model-> this study proposed heterogenous form of group pedestrian into the model by varying pedestrian social relationship denoted as $T_g$ 
    ...
    - Based on the pedestrian group types above $T_{g} \in (T_{gfd}, T_{gcl}, T_{gfy}, T_{ged})$, the modified social force model for pedestrian group is formulated as shown below:

    $$
    \hat{f}_i^g(t) = - \beta_1(T_g)\alpha_i \hat{v_i} + q_A\beta_{2}(T_g)\hat{U_i}+\sum_k q_{R} \beta_{3}(T_g) \hat{W_{ik}}
    $$ 

    where:
    |parameter|description|
    |---------|-----------|
    |$\beta_{1}$|the strength coefficient of the field-of-view force|
    |$\beta_{2}$|the strength coefficient of the mutual attractive force|
    |$\beta_{3}$|the strength coefficient of the repulsive force|
    |$v_{free}$|the desired velocity of the pedestrian|
    |$d_0$|comfor distance|
    |$\alpha_1$|rotation angle|
    |$q_A$|the exponential function|
    |$q_R$|piecewise function|
    |$\hat{U_i}$|the unit vector pointing from pedestrian $i$ to the centroid $C_i$|
    |$\hat{W_{ik}}$|the unit vector pointing from pedestrian $i$ to group member $k$|
    
    - **Evasion maneuveurs**-> collective avoidance forces meaning that a group of pedestrian preceed avoidance maneuver collectively comprising (a) unified avoidance and (b) inter-weavering avoidance -> proposed by this study as accompanying pedestrian avoidance force $(f_{gr})$

- Result indicates comparison parameter between simulated (80% of samples) and validated/observed (20% of samples) based on these aspects:
    - pedestrian speed distribution
    - pedestrian spatial density distribution
    - the relationship between velocity and density
    - pairing group identification comparison
    - evasion frequency
...
- continue to `2026-05-01.md` file
